...
ViberClient.prototype.sendMessage = function(toViberUserId, messageType, messageData, optionalTrackingData, optionalKeyboard) {
	if (!toViberUserId) {
		throw new Error(`Invalid arguments passed to sendMessage. 'toViberUserId' is invalid.`);
	}
	...
	if (!messageType && !messageData && !optionalKeyboard) {
		throw new Error(`Invalid arguments passed to sendMessage. 'MessageData','messageType' is Missing and there's no keyboard.`);
	}
	...
	this._logger.debug("Sending %s message to viber user '%s' with data", messageType, toViberUserId, messageData);
